...
public class MappingEvaluator extends JpqlVisitorAdapter<Set<TypeDefinition>> {
    ...
    public MappingEvaluator(Metamodel metamodel, SecurityContext securityContext) {
        super(new HashSet<TypeDefinition>());
     ...
     */
    public void evaluate(ParserRuleContext node) {
        this.visit(node);
    }
    ...
    public boolean visit(JpqlSubselect node, Set<TypeDefinition> typeDefinitions) {
        ...
        for (int i = 1; i < node.getChildCount(); i++) {
            node.getChild(i).visit(this, subselectDefinitions);
        }
        // visit the select clause last
        node.getChild(0).visit(this, subselectDefinitions);
        return false;
    }

    @Override
    public boolean visit(JpqlFromItem node, Set<TypeDefinition> typeDefinitions) {
        String typeName = node.getChild(0).toString().trim();
        Alias alias = new Alias(node.getChild(1).toString().trim());
    ...
    public boolean visitJoin(ParserRuleContext node, Set<TypeDefinition> typeDefinitions) {
        if (node.getChildCount() != 2) {
            return false;
        }
        Node pathNode = node.getChild(0);
        Node aliasNode = node.getChild(1);
        Alias rootAlias = new Alias(pathNode.getChild(0).toString());
        Class<?> rootType = getType(rootAlias, typeDefinitions);
        ManagedType<?> managedType = forModel(metamodel).filter(rootType);
        for (int i = 1; i < pathNode.getChildCount(); i++) {
            Attribute<?, ?> attribute = managedType.getAttribute(pathNode.getChild(i).toString());
            if (attribute.getPersistentAttributeType() == PersistentAttributeType.BASIC) {
                throw new PersistenceException("Cannot navigate through basic property "
                        + pathNode.getChild(i) + " of path " + pathNode);
            }
